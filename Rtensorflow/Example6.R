...
df <- read.csv("/home/konrad/Documents/GitHub/ds/Rtensorflow/qsar_aquatic_toxicity.csv", sep = ";")
names(df) <- c("TPSA(Tot)", "Sacc", "H-050", "MLOGP", "RDCHI", "GATS1p", "nN", "C-040", "LC50")
AquaticToxicity <- df

# Normalize the input features to have zero mean and unit variance
x <- scale(AquaticToxicity[, -9])
y <- AquaticToxicity[, 9]
